...
##############################################################################
#
#   IMPORTANT: This file uses the Portuguese Student dataset, which is a well
#              known dataset used for Machine Learning examples.
#
##############################################################################
...
######################################
set.seed(12345) # set seed for repeatable sampling
percentCover <- 70
s <- sample(dim(dStudent.portuguese)[1], floor(percentCover/100*dim(dStudent.portuguese)[1]))


#############################################################################
# Start Decision tree classification (Single training / Single test Set)
#                         NO cross validation
#############################################################################

# Split the for training and test data
student_train <- dStudent.portuguese[s,]
student_test <- dStudent.portuguese[-s,]

#rpart to do the decision tree model
# to predict the classification of species

# Run the Decicion Tree Classification algorithm using all variables as predictors
dtm <- rpart(G3 ~ ., student_train, method="class")

# plot the decision tree
rpart.plot(dtm)

predictions <- predict(dtm, student_test, type="class")
...
student_train <- dStudent.portuguese # Use the whole set for training with k-folds
student_test <- dStudent.portuguese[-s,]
...
# Setup the doSNOW package for multithreading for training many trees
...
# Use the rpart.plot to plot the final model
rpart.plot(student_train.model$finalModel)

# make the predictions a
